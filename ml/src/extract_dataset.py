...
def load_and_flatten(json_path: Path) -> tuple[list[dict], int, int]:
    """Load a JSON file and flatten each record.

    Returns:
        (rows, total_records, excluded_count)
    """
    ...
    return rows, len(records), excluded
...
def extract_dataset() -> pd.DataFrame:
    ...
    for json_path in files:
        rows, n_records, n_excluded = load_and_flatten(json_path)
        all_rows.extend(rows)
        total_records += n_records
        total_excluded += n_excluded
        algo = json_path.parent.name
        size = json_path.stem
        status = f"  [{algo}/{size}.json] {n_records} records → {n_excluded} excluded, {len(rows)} kept"
        print(status)

    print(f"\nTotal records: {total_records}  |  Excluded: {total_excluded}  |  Kept: {len(all_rows)}")
    ...
    # ---- Assign labels ----
    # Import here (after path setup) to keep modules decoupled
